...
final class ContextSynchronizationTests: BaseTestCase {
    ...
    func syncTestBasic() async throws {
        ...
        b.performAndWait {
            _ = Book.findOrCreate(id: "hello", context: b)
            _ = Book.findOrCreate(id: "hello2", context: b)
        }
        
        await b.perform(schedule: .enqueued, { [b] in
            _ = Book.findOrCreate(id: "hello3", context: b)
            _ = Book.findOrCreate(id: "hello4", context: b)
        })
    ...
    func syncTestExtended() async throws {
        ...
        b.performAndWait {
            _ = City.findOrCreate(id: "nyc-1", context: b)
            _ = Language.findOrCreate(column: "langCode", value: "en-us", context: b)
        }
        
        await b.perform(schedule: .enqueued, { [b] in
            _ = City.findOrCreate(id: "ldn-1", context: b)
            _ = Language.findOrCreate(column: "langCode", value: "jp", context: b)
        })
